...
<html lang="en">
  <head>
    ...
    <title>Dept Head | Apply for Leave</title>
    <link
    ...
    <main class="flex-grow p-4 overflow-y-auto mt-6">
      ...
      <div class="bg-white rounded-xl shadow-md p-6 mb-6">
        ...
        >
          <!-- Card: Vacation Leave -->
          <a
            href="leave_form.php?type=Vacation Leave"
          ...
          <!-- Card: Sick Leave -->
          <a
            href="leave_form.php?type=Sick Leave"
          ...
          <a
            href="leave_form.php?type=Mandatory Forced Leave"
          ...
          <!-- Card: Study Leave -->
          <a
            href="leave_form.php?type=Study Leave"
          ...
          <a
            href="leave_form.php?type=Special Emergency (Calamity) Leave"
          ...
          <!-- Card: Adaption Leave -->
          <a
            href="leave_form.php?type=Adaption Leave"
    ...
    <script>
      ...
      (function () {
        async function resolveUserEmail() {
          try {
            ...
            if (js && js.logged_in && js.email) {
              console.log('[DEPT HEAD] API returned email:', js.email);
              try {
                // Update localStorage with fresh email
                localStorage.setItem("userEmail", js.email);
              } catch (e) {
                console.error('[DEPT HEAD] Failed to set localStorage:', e);
              }
              return js.email;
            }
            console.error('[DEPT HEAD] API returned invalid data:', js);
          } catch (e) {
            console.error('[DEPT HEAD] Exception in resolveUserEmail:', e);
          }
          return "";
        }
        ...
        async function initLeaveApplication() {
          console.log('[DEPT HEAD] Initializing leave application check...');
          const email = await resolveUserEmail();
          console.log('[DEPT HEAD] Resolved email:', email);
          if (!email) {
            console.error('[DEPT HEAD] No email found, cannot check leave status');
            return;
          }
          ...
          try {
            ...
            if (!statusResp.ok) {
              console.error('[DEPT HEAD] API response not OK:', statusResp.status);
              // Continue with default (enabled) if API fails
            } else {
              const statusData = await statusResp.json();
              console.log('[DEPT HEAD] Status data:', statusData);
              if (statusData && statusData.success) {
                canApplyLeave = statusData.data.can_apply_leave === 1;
                console.log('[DEPT HEAD] Can apply leave:', canApplyLeave);
              } else {
                console.error('[DEPT HEAD] API returned error:', statusData);
              }
            }
          } catch (e) {
            console.error("[DEPT HEAD] Failed to fetch leave application status", e);
          }

          console.log('[DEPT HEAD] Final canApplyLeave value:', canApplyLeave);

          // If user cannot apply for leave, show warning and disable all cards
          if (!canApplyLeave) {
            console.log('[DEPT HEAD] DISABLING LEAVE APPLICATION');
            const container = document.getElementById('leave-cards');
            console.log('[DEPT HEAD] Container found:', container);
            if (container) {
            ...
            console.log('[DEPT HEAD] All cards disabled');
          ...
          console.log('[DEPT HEAD] Showing cards after data load');
